...
*/
				...
				else if(response[0].compareTo("whoami") == 0  && logged == true)
				{
					...
					System.out.println("About to enter observer finder");
					...
					System.out.println("Observer test done... true/false: " + observer);					
					if (observer)
					{
					...
					System.out.println("after observer find");
					send += " My unique ID is: " + me.getThreadId() + newline;
					send += " Remote address: " + me.getSocket().getInetAddress().getLocalHost() + newline;
					send += " Remote port is: " + me.getSocket().getLocalPort() + newline;
					send += " Local address is: " + me.getSocket().getInetAddress().getHostName() + " {" +
				...
				else if (response[0].compareTo("remove") == 0  && logged == true)
				{
					...
					{
						send += "CROSBY87 400 ERROR Hold your 'horeses' - You are not in a game, or its not your turn." +newline;
						sendString(send, this.connection);
					}
				}
